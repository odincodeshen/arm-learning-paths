...
In the Workbench for Zephyr panel:
...
Click the **Build** button in Workbench for Zephyr or press `Ctrl+Shift+B`.
...
Once installed, Workbench for Zephyr will attempt to detect it automatically during a debug session.
If you’re using a different board, refer to your vendor’s documentation to install the appropriate debug utility.

{{% notice Note %}}
If Workbench for Zephyr does not automatically detect the installed debug runner, you can manually configure it.
